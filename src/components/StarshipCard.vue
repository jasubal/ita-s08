...
<div id="c-starship" >
<div id="starship" v-if="showcard" >
<header class="flex center two">
<h1>{{name}}</h1>
<div id="imagWrapper">
<img id="shipImg" :src="imgUrl" :alt="name" @error="setAltImg">
</div>
</header>
    <ul id="s-features" class="flex center">
        <li><span>Model: </span> {{model}} </li>
        <li><span>S Class:  </span>{{s_class}}</li>
        <li><span>Crew:  </span>{{crew}}</li>
        <li><span>Passengers:  </span>{{ passengers  }}</li>
        <li><span>Cargo Capacity:  </span>{{ cargo_capacity  }}</li>
        <li><span>Length:  </span>{{ length  }}</li>
        <li><span>Hyperdrive Rating:  </span>{{ Hyperdrive  }}</li>
        <li><span>Consumables: </span>{{ consumables  }}</li>
        <li><span>Cost In Credits:  </span>{{ cost_in_credits  }}</li>
        ...
        <li v-if="getPilots.length > 0" id="c-pilots"> <Pilots />  </li>
    </ul>
...
#c-starship li { background: #222; color:var(--c-sws); margin: 2px; padding: 6px; text-align: left;font-weight: bold; }
#c-starship li span{color:var(--c-w); display: block;font-weight: 600; }
#starship {overflow: hidden;}
#s-features { margin-left: 0; }
#c-pilots {
    width: 100%;
}
